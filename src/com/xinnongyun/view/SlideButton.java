...
//         slideButtonBG = Bitmap.createBitmap(slideButtonBG, 0, 0, slideButtonBG.getWidth(), 
//        		 slideButtonBG.getHeight(), matrix, true); 
//         switchBG = Bitmap.createBitmap(switchBG, 0, 0, switchBG.getWidth(), 
//        		 switchBG.getHeight(), matrix, true); 
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isSliding) {
			...
			if (currentState) {
				// 绘制打开状态
				canvas.drawBitmap(slideButtonBG, switchBG.getWidth() - slideButtonBG.getWidth() - (float)1.5, 0, null);
			} else {
				// 绘制关闭状态
				canvas.drawBitmap(slideButtonBG, (float)1.5, 0, null);
			}
		}
	}
